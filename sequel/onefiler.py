...
class Molecule:
    ...
    @property
    def atoms_dict(self) -> dict[str, int]:
        numbers = {}
        for atom in self.__atoms:
            if atom.element not in numbers:
                numbers[atom.element] = 1
...
class ParseHer:
    ...
    def __init__(self, name: str) -> None:
        ...
        self.molecule = Molecule(self.name)

    def parse(self) -> dict[str, int]:
        chain_type = self.find_chain()
        radical = self.find_radical()

        self.molecule.brancher(radical)

        for i in range(chain_type):
            self.molecule.bounder((1, 1, 2, 1))

        self.molecule.closer()
        return self.molecule.atoms_dict
    ...
    def find_radical(self) -> int:
        if self.working_name == "":
            return None

        for i in range(len(self.working_name)):
            radical = self.working_name[i:]
            if radical in self.RADICALS:
                self.working_name = self.working_name[:i]
                return self.RADICALS.index(radical) + 1
            
        return None

    def find_chain(self) -> int:
        if self.working_name == "":
            return None

        chain = self.working_name[len(self.name) - 3:]
        if chain not in self.CHAINS:
            return None

        self.working_name = self.working_name[:-3]
        return self.CHAINS.index(chain)


if __name__ == '__main__':
    p = ParseHer("2,3-dimethylhexene")
    print(p.parse())
    print(p.working_name)
